...
describe("settings", () => {
  ...
  it("normalizes custom printer defaults", () => {
    ...
    expect(defaults.selectedPrinterId).toBe("custom");
    expect(defaults.input.bedWidth).toBe(280);
    expect(defaults.input.bedDepth).toBe(260);
    expect(defaults.input.includeMagnets).toBe(false);
    expect(defaults.input.openBottom).toBe(true);
  ...
  it("uses preset bed dimensions for saved presets", () => {
    ...
    expect(defaults.input.bedWidth).toBe(305);
    expect(defaults.input.bedDepth).toBe(320);
  });
});
